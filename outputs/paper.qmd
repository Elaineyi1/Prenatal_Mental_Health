...
Two new binary variables named `depression` and `anxiety` are created. Mothers scoring above 12, which represents 40% on the EPDS scale, are likely to be suffering from depression and should seek medical attention [@EDPSstanford]. Consequently, the variable `depression` is assigned to a value of 1 if the EPDS score is greater than or equal to 0.4, and 0 otherwise. Similarly, the `PROMIS` score of at least 20 indicates at least moderate depression. Hence, the variable `anxiety` is assigned to 1 if the `PROMIS` is larger or equal to 0.464, and 0 if it is smaller than 0.464. 
...
clean_data |>
  ...
  scale_fill_manual(values = c("0" = "#f4bfb6", "1" = "#f09286"), 
                    labels = c("No or Mild Anxiety", "Moderately Anxiety or more")) +
  coord_flip() +
...
```
\newpage
Three questions specific to fears of COVID-19 were asked in the survey. Answers were on a continuous scale from 0 (“not at all”) to 100 (“very much so”), with 50 representing “somewhat” [@nihPrenatalMental].
