...
depression_model <-
  stan_glm(
    depression ~ age_group + household_income + language + maternal_education + threaten_life + threaten_baby_danger,
    data = clean_data,
...
anxiety_model <-
  stan_glm(
    anxiety ~ age_group + household_income + language + maternal_education + threaten_life + threaten_baby_danger,
    data = clean_data,
    family = binomial(link = "logit"),
    prior = normal(location = 0, scale = 2.5, autoscale = TRUE),
    prior_intercept = 
